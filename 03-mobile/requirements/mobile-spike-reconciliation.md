...
| SPIKE-003 | Evaluate Barcode / QR / GS1 scanning | Validate whether camera/scanner input is useful for each task, identifier mapping, QR TTL/replay and future GS1 boundary. | PARTIAL; API v0.17.0 supports identifier/handoff contracts, device research pending; scanning is not universal. |
| SPIKE-004 | Evaluate Mobile local storage and offline synchronization | Research connectivity needfinding and define safe cache/draft/evidence staging, retry, conflict and recovery behavior per story. | PARTIAL target; no client implementation, no offline critical success or generic V1 sync. |
