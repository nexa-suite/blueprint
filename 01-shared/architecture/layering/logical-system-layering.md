...
This layering model is sufficient to explain the current system and to guide later refactoring decisions, but it is not a claim that every class conforms. The API-specific evidence and classifications are in [API layering principles](api-layering-principles.md); tenant context is in [Multi-tenant context propagation](../../security/multi-tenancy/context-propagation.md).
