...
| Dispatch handoff | `S` verify goods/packages → assign Driver → explicit Dispatch Handoff + stable ID and optional policy-authorized evidence → dispatch; `A` handoff notification and traceability |
| Delivery / POD | `S` attempt → offered/accepted/rejected quantities → recipient acknowledgement + optional policy-authorized evidence + POD; `A` DeliveryCompleted or ContinuationDeliveryCreated |
...
dispatch actor, timestamp, optional policy-authorized evidence reference and
handoff ID. POD contains Delivery Attempt ID, recipient identity evidence,
timestamp, delivered/rejected quantities and reasons, optional
policy-authorized evidence reference, actor and evidence references. V1 uses
external navigation handoff only; it does not persist Driver location. A Buyer
QR token is ephemeral, opaque, time-limited and replay-protected; it does not
