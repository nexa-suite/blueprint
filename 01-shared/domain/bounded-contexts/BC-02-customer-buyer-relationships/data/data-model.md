...
|---|---|---|
| `customer_account` | `customer_account_id` | `tenant_id, workspace_id` NOT NULL; scoped candidate key `(customer_account_id, tenant_id, workspace_id)`; tax identifier scoped unique |
...
| `buyer_relationship` | `relationship_id`; composite Customer Account scope FK | tenant/workspace scope must match Customer Account; customer + human identity unique |
...
`human_identity_id` and actor IDs are cross-BC stable references. The local
`BuyerRelationship -> CustomerAccount` relation uses the scoped candidate key,
so it cannot cross Tenant or Workspace. `customer_contact`, `customer_address`
and relationship history are parent-derived simple children; they do not
duplicate scope columns. Customer account, contact, address and relationship
lifecycle is BC-02-owned; history is append-only. AS-IS: `sales.client_account`, `client_account_address`,
