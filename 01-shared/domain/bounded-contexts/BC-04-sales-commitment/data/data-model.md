...
| Commitment | `commercial_commitment`, `commercial_commitment_line`, `commitment_owner_transfer`, `sales_commitment_adjustment` | scoped Purchase Request FK; `origin_type` requires either a real Purchase Request or a Direct Order with no PR |
| Sales Order | `sales_order`, `sales_order_line` | scoped Commitment FK; SO is born `CONFIRMED`; order line pair unique |
...
`PurchaseRequest -> CommercialCommitment -> SalesOrder` uses local composite
candidate keys/FKs, so same-context root links cannot cross Tenant or
Workspace. Lines, proposals, transfers and adjustments are parent-derived
simple children and do not duplicate scope. `buyer_relationship_id`, `sku_id`, identity IDs and downstream decision IDs
