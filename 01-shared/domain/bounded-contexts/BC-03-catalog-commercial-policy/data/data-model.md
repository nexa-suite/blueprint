...
|---|---|---|
| Catalog | `product`, `sku`, `catalog_media` | product -> SKU/media composition; scoped codes; status checks; optional GTIN resolution index |
...
sellable variant. `sku.gtin` is the current target's physical identifier
projection; the application rejects ambiguous scoped matches and does not
create a Scanner/QR table or aggregate. AS-IS anchors:
`catalog_management.product`, family, variant, sellable SKU, GTIN, prices,
visibility, promotion and asset tables. V40, V48, V54, V65 and v0.17.0 V97
are historical/current implementation evidence, not target schema authority.
See [product participation](../../../../data/product-data-participation.md).
