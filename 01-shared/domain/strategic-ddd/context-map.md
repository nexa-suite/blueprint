...
| Credit & Receivables | Business Documents | Customer/Supplier / Published Language | committed receivable and financial-adjustment source facts | durable async fact | immutable financial snapshot | eventual after source commit |
| Payments | Business Documents | Customer/Supplier / Published Language | confirmed/reconciled payment facts and payment-receipt source facts | durable async fact | provider-neutral payment reference | eventual after source commit |
...
| Payments | Payment Provider (external) | Anti-Corruption Layer | provider request/webhook translation | external I/O after intent/commit boundary | provider ACL; no provider vocabulary in domain | reconciliation and deduplication explicit |
